...
protected:
	...
	{
		return _hasher->getHash(item, _buckets.size() - 1);
	}
	...
	virtual bool needsResize()
	{
		...
		if (_number_of_elements > (0.5 * _primes[_local_prime_index]))
		{
			return true;
		}
		return false;
	...
	virtual void resizeCheck()
	{
		...
		if (needsResize())
		{
			...
			BucketHashTable<Key, Value> new_hash{ hasher, _primes[_local_prime_index] };
